...
3. 닉네임에 특수문자나 이모지가 포함될 수 있습니다 — 정확히 추출하세요.
4. 멤버목록 스크린샷에는 보통 닉네임, 전투력, FC레벨이 있습니다.
5. 지심탐험 스크린샷에는 순위와 닉네임이 있습니다.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const members = parsed.map((m, i) => ({
      ...
      rank: i + 1,
      nickname: m.nickname || `Unknown_${i}`,
