      ...
      <div className="p-4 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {DEFAULT_COUNTER_MATRIX.map((entry, i) => (
            <div key={i} className="flex items-center gap-2 text-xs sm:text-sm p-2 bg-gray-100 dark:bg-gray-800 rounded">
              <span className="text-red-600 dark:text-red-400">{entry.enemyCompKo} ({entry.enemyRatio})</span>
              <span className="text-gray-400 dark:text-gray-500">&rarr;</span>
              <span className="text-green-600 dark:text-green-400">{entry.ourResponseKo} ({entry.ourRatio})</span>
            </div>
          ))}
        </div>
      </div>
            ...
            <tbody>
              {DEFAULT_JOINER_EFFECTS.map((effect, i) => (
                <tr key={i} className="border-t border-gray-200 dark:border-gray-800">
                  ...
                  <td className="py-2 text-gray-600 dark:text-gray-300">{effect.effectKo}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function RallyTypeCard({
  ...
  return (
    <div className={`p-3 sm:p-4 rounded-lg border ${colors.border} ${colors.bg}`}>
      ...
      {/* Leader Composition */}
      <div className="mb-3">
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">집결장 영웅</p>
        {isEditing ? (
          <div className="space-y-1">
            {(['hero1Id', 'hero2Id', 'hero3Id'] as const).map((slot, i) => (
              <select
                ...
                className="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1 text-xs sm:text-sm text-gray-800 dark:text-gray-200"
              >
                {HEROES.map((h) => (
      ...
      {/* Joiner */}
      <div className="mb-3">
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">집결원</p>
        {isEditing ? (
          <div className="space-y-1">
            {rally.joinerHeroes.map((heroId, i) => (
              <div key={i} className="flex items-center gap-1">
                <select
                  ...
                  className="flex-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1 text-xs sm:text-sm text-gray-800 dark:text-gray-200"
                >
                ...
                {rally.joinerHeroes.length > 1 && (
                  <button
                    ...
                    className="text-red-600 dark:text-red-400 text-xs px-1"
                  >
                    X
                  </button>
                )}
...
function RatioSlider({
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      <span className="w-6 sm:w-8 text-gray-600 dark:text-gray-300">{label}</span>
      <input
        type="range"
        min={0}
        max={10}
      ...
      />
      <span className="w-6 text-right text-gray-500 dark:text-gray-400">{value}</span>
